...
static ELFObjectContext* elfctx_new(
    jit_string_t* filename,
    int lineno,
    jit_string_t* function_name,
    void* code,
    int code_size,
    int stack_size);
...
register_elf_ctx(ELFObjectContext* ctx, const char* type, void* ptr) {
  ...
  char* raw =
      (char*)(calloc(1, sizeof(JITCodeEntry) + elf_object_size));
  if (raw == NULL) {
...
typedef struct ELFObjectContext {
  ...
  jit_string_t* function_name; /* The python function name */
  jit_string_t* filename; /* The python file the function came from */
...
static void elf_init_ehframe(ELFObjectContext *ctx)
{
  ...
  /* Emit DWARF EH CIE. */
  DWRF_SECTION(CIE,
    DWRF_U32(0);			/* Offset to CIE itself. */
    ...
    DWRF_U8(1); DWRF_U8(DWRF_EH_PE_textrel|DWRF_EH_PE_udata4);  /* Augmentation data. */
  ...
  DWRF_SECTION(FDE,
...
#ifdef __x86_64__
    DWRF_U8(DWRF_CFA_offset|DWRF_REG_BP); DWRF_UV(2);
    DWRF_U8(DWRF_CFA_offset|DWRF_REG_BX); DWRF_UV(3);
    DWRF_U8(DWRF_CFA_offset|DWRF_REG_15); DWRF_UV(4);
    DWRF_U8(DWRF_CFA_offset|DWRF_REG_14); DWRF_UV(5);
    /* Extra registers saved for JIT-compiled code. */
      ...
      DWRF_U8(DWRF_CFA_def_cfa_offset); DWRF_UV(0);
#else
#error "Unsupported target architecture"
#endif
