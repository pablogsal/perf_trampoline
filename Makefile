...
pycoverage:  ## Run the test suite, with Python code coverage
	$(PYTHON) -m pytest -vvv --log-cli-level=info -s --color=yes \
				--cov=perf_trampoline --cov=tests --cov-config=pyproject.toml --cov-report=term \
				--cov-append $(PYTEST_ARGS) tests --cov-fail-under=92
	$(PYTHON) -m coverage lcov -i -o pycoverage.lcov
	genhtml *coverage.lcov --branch-coverage --output-directory perf_trampoline-coverage
...
ccoverage:  ## Run the test suite, with C++ code coverage
	...
	$(MAKE) check
	gcov -i build/*/src/perf_trampoline/ -i -d
	lcov --capture --directory .  --output-file cppcoverage.lcov
	lcov --extract cppcoverage.lcov '*/src/perf_trampoline/*' --output-file cppcoverage.lcov
	genhtml *coverage.lcov --branch-coverage --output-directory perf_trampoline-coverage
...
lint:  ## Lint all files
	$(PYTHON) -m pre_commit run --all-files
	$(PYTHON) -m mypy src/perf_trampoline --strict --ignore-missing-imports
...
clean:  ## Clean any built/generated artifacts
	...
	rm -rf build
	rm -f src/perf_trampoline/*.so
	rm -f {cpp,py}coverage.lcov
	rm -rf perf_trampoline-coverage
...
gen_news: check_release_env
	$(eval CURRENT_VERSION := $(shell bump2version \
	                            ...
	                            | sed s,"^.*=",,))
	$(PYEXEC) towncrier build --version $(CURRENT_VERSION) --name perf_trampoline
